...
export const parseRoofXML = (xmlString: string): RoofData => {
  const options = {
    ...
    isArray: (name: string) =>
      ["FACE", "LINE", "POINT", "POLYGON"].includes(name),
  };
  ...
  const cleanFloat = (val: unknown): number => {
  ...
  (roof.POINTS?.POINT || []).forEach((point: any) => {
    ...
    if (typeof point.data === "string") {
      const cleanData = String(point.data)
        .replace(/\u2212/g, "-")
        .replace(/−/g, "-");
      const coords = cleanData
        .split(",")
  ...
  (roof.LINES?.LINE || []).forEach((line: any) => {
    if (line?.id && line.path) {
      linesMap.set(
        line.id,
        line.path.split(",").map((s: string) => s.trim())
      );
    }
  });
  ...
  (roof.FACES?.FACE || []).forEach((face : any) => {
    const polygon = Array.isArray(face.POLYGON)
      ? face.POLYGON[0]
      : face.POLYGON;
    ...
    lineIds.forEach((lineId: string, index: number) => {
  ...
  (roof.LINES?.LINE || []).forEach((line: any) => {
    ...
    const path = pIds
      .map((pid: string) => pointsMap.get(pid))
      .filter((p: unknown): p is Point => p !== undefined);
    if (path.length >= 2) {
