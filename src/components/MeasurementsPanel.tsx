...
export const MeasurementsPanel = ({ faces, lines, activeFileName, selectedFaceIds, faceId }: MeasurementsPanelProps) => {
      ...
      <div className="p-5 border-b border-gray-200 bg-gray-50">
        ...
        {activeFileName ? (
           <div className="mt-2 text-sm flex items-center gap-2 text-blue-600 bg-blue-50 p-2 rounded border border-blue-100">
             <span>📄</span> 
             <span className="font-medium truncate" title={activeFileName}>{activeFileName}</span>
           </div>
        ) : (
      ...
      <div className="flex-1 overflow-y-auto bg-white"> 
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {faces.map((face) => (
                ...
                <tr key={face.id} className="hover:bg-blue-50 transition-colors">
                  <td className="p-3 font-medium text-blue-600" >{face.designator}</td>
                  <td className="p-3">{face.pitch}°</td>
                  <td className="p-3 text-right font-mono">{Math.round(face.size)}</td>
                </tr>
              ))}
            </tbody>
          </table>
               ...
               <tr key={line.id} className="hover:bg-gray-50">
                 <td className="p-3">
                   <span className={`px-2 py-1 rounded text-xs font-bold uppercase
                     ${line.type === 'RIDGE' ? 'bg-red-100 text-red-700' : 
                       line.type === 'EAVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}
                   `}>
                     {line.type}
                   </span>
                 </td>
        ...
        {activeTab === 'summary' && (
          <div className="p-4 grid grid-cols-2 gap-3">
            <SummaryCard label="Total Area" value={`${Math.round(totalArea)} sq ft`} />
...
const SummaryCard = ({ label, value }: { label: string, value: string | number }) => (
  <div className="bg-gradient-to-br from-purple-600 to-indigo-600 rounded-xl p-4 text-white shadow-md">
    <div className="text-xs text-purple-200 uppercase font-bold tracking-wider mb-1">{label}</div>
