  ...
  const { geometry, edges, centerPoint } = useMemo(() => {
    if (!face.path || face.path.length < 3) {
      return {
        ...
        edges: [] as [THREE.Vector3, THREE.Vector3][],
        centerPoint: null as THREE.Vector3 | null,
      };
    }
    ...
    <group>
      ...
      >
        <meshStandardMaterial
          ...
          flatShading={true}
          emissive={
            isSelected ? new THREE.Color(0x2b7bbf) : new THREE.Color(0x000000)
          }
      ...
      {edges.map((edge, i) => (
        <Line
          key={i}
          points={[edge[0].toArray() as unknown as THREE.Vector3, edge[1].toArray() ]}
          // points={[edge[0].toArray() as unknown , edge[1].toArray() as any ]}
          lineWidth={1.5}
          color="#2d3436"
        />
      ))}
...
export const Roof3DView = ({
  ...
  if (!faces || faces.length === 0) {
    return (
      <div className="bg-red-500 flex justify-center align-center h-full">
        No Data
      </div>
    );
  }
  ...
  return (
    <Canvas camera={{ position: [50, 60, 50], fov: 45 }} shadows>
